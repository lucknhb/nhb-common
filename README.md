...
<font color='red'>底层依赖Redission开源版</font>
...
<dependency>
   <groupId>com.nhb</groupId>
   <artifactId>nhb-common-sensitive</artifactId>
   <version>${version}/使用引入BOM方式时无需填入版本号</version>
</dependency>
```

@Sensitive注解作用在字段上，属性如下

strategy ：脱敏策略  SensitiveStrategy枚举

roleKeys ： 角色标识符 (需要实现SensitiveService接口中的isSensitive方法 返回true表示需要脱敏 false表示不需要脱敏)
...
<dependency>
   <groupId>com.nhb</groupId>
   <artifactId>nhb-common-rocketmq</artifactId>
   <version>${version}/使用引入BOM方式时无需填入版本号</version>
</dependency>
```

该模块底层依赖rocketmq-client

使用@RocketMQConsumer注解声明消费者 该注解作用在方法上，注解包含以下属性

consumeMode ： 消费模式 默认CONCURRENTLY（并发模式消费）另外一种为 ORDERLY(顺序消费)

topic ： 需要消费的topic 尽量保持一个 Topic 一种消息类型

tags ： 过滤的表达式,*代表订阅所有tag,多个tag使用||连接

consumerGroup ：  消费者组名称 默认 NHB_CONSUMER_GROUP

<font color='red'>注意：使用@RocketMQConsumer的目标方法参数：1. 当且仅当参数为一个时并且非元数据类型(MessageExt)时，会将消息内容使用反序列化工具转换为具体对象  2. 其他情况下可存在单个或多个以下类型参数: MessageExt(元数据类型)、byte[]类型(消息内容)、String类型(消息内容转为字符串文本)</font>

关于模块中所需要的配置项如下

```yaml
rocketmq:
  #是否开启使用 spring.profiles.active 拼接到topic后缀
  #为true时  topic_dev/topic_test 默认为true
  profileEnabled: true
  #消息内容序列化方式 提供FORY/JACKSON两种方式 默认为FORY
  #对于自定义序列化方式 实现com.nhb.common.rocketmq.converter.MessageConverter该接口并加到IOC容器中即可
  converter-type: FORY
  #生产者
  producer:
    name-server-address: #服务地址端口
    #生产者组名称 默认为 NHB_PRODUCER_GROUP
    producer-group: NHB_PRODUCER_GROUP
    #发送超时时间 单位 MS  默认3000
    send-message-timeout: 3000
    #实例名称 默认 NHB_INSTANCE
    instance-name: ${spring.application.name}
    #是否开启VIP通道 默认否
    vip-channel-enabled: false
    #topic队列数量 默认4
    topic-queue-number: 4
    #是否使用TLS 默认false
    useTLS: false
    #当设置为 true 时，客户端在拉取消息等核心操作中会启用单元化相关的处理逻辑 默认false
    unit-mode: false
    #消费者将只会消费那些发送时指定了相同单元名称的消息 当开启unit-mode=true时生效
    #unit-name:
  #消费者
  consumer:
    name-server-address: #服务地址端口
    #消费者组 默认NHB_CONSUMER_GROUP
    consumer-group: NHB_CONSUMER_GROUP
    #实例名称 默认NHB_INSTANCE
    instance-name: NHB_INSTANCE
    #消费超时时间 单位S 默认15S
    consume-timeout: 15
    #最大重试次数 PUSH 模式下有效 默认3
    max-reconsume-times: 3
    #消费模式,默认为集群模式
    consume-message-model: CLUSTERING
    #当设置为 true 时，客户端在拉取消息等核心操作中会启用单元化相关的处理逻辑 默认false
    unit-mode: false
    #消费者将只会消费那些发送时指定了相同单元名称的消息 当开启unit-mode=true时生效
    #unit-name:
    #PULL模式下拉取超时时间 默认10S
    consumer-pull-timeout-millis: 10000
    #是否开启VIP通道 默认否
    vip-channel-enabled: false
```

模块中提供 了RocketMQClientUtil工具类里面包含发送消息方法/获取序列化方式/直接序列化及反序列化方法/获取生产者方法
