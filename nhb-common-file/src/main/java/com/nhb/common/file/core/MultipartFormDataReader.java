 ...
 * @version 1.0
 * @date 2026/3/9 22:28
 * @description multipart/form-data 读取器
 */
public class MultipartFormDataReader {
    ...
    public static MultipartFormData read(
        ...
        if (boundary == null) {
            //HttpServletRequest 的 ContentType 中未读取到 boundary 参数
            throw new FileStorageException("HttpServletRequest  ContentType Not Found Boundary Params！");
        }
    ...
    public static int readPart(MultipartFormData data) throws IOException {
        ...
        if (StrUtil.isEmpty(disposition)) {
            //HttpServletRequest 的 Part 无法识别 content-disposition
            throw new FileStorageException("HttpServletRequest  Part Unrecognizable Content-Disposition");
        }
    ...
    public static byte[] readLineBytes(MultipartFormData data) throws IOException {
        ...
        if (readLength == -1) {
            //HttpServletRequest 解析失败，尚未发现文件
            throw new FileStorageException("HttpServletRequest Parsing Failed NoFiles Have Been Found");
        }
        data.totalReadLength += readLength;
        if (readLength == data.buffer.length) {
            //HttpServletRequest 解析失败，参数超过缓冲区大小
            throw new FileStorageException("HttpServletRequest Resolution Fails And The Parameter Exceeds The Buffer Size");
        }
    ...
    public static class MultipartFormDataPartInputStream extends InputStream {
        ...
        protected void readLineBuffer() throws IOException {
            ...
            int readLength = readLine(data.inputStream, data.buffer, 0, data.buffer.length);
            if (readLength == -1) {
                //HttpServletRequest 解析失败，文件尚未完整读取
                throw new FileStorageException("HttpServletRequest The Resolution Fails, And The File Has Not Been Fully Read");
            }
