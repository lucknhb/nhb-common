...
public class RemoteFileInfo {
    ...
    public FileInfo toFileInfo(FileInfo fileInfo) {
        ...
        fileInfo.setMetadata(ToolUtil.toStringMap(metadata));
        fileInfo.setUserMetadata(ToolUtil.toStringMap(userMetadata));
        return fileInfo;
    }
    ...
    public FileInfo toFileInfoTh(FileInfo fileInfo) {
        fileInfo.setThumbnailFileName(filename).setThumbnailUrl(url).setThumbnailSize(size).setThumbnailContentType(contentType);
        // RemoteFileInfo 中的 metadata 元数据值为 Object 类型，FileInfo 中的元数据值为 String 类型，
        // 目前是将 Object 类型使用 toString() 方法转为 String 类型，应该可以满足大部分情况，如有需要可以自行转换
        fileInfo.setThumbnailMetadata(ToolUtil.toStringMap(metadata));
        fileInfo.setThumbnailUserMetadata(ToolUtil.toStringMap(userMetadata));
        return fileInfo;
    }
