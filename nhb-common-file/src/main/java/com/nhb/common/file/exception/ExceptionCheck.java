 ...
 * @version 1.0
 * @date 2026/3/9 15:01
 * @description: 用于检查条件并抛出对应异常，主要用于存储平台的实现类中
 */
public class ExceptionCheck {
    ...
    public static void uploadPart(FileInfo fileInfo) {
        if (fileInfo == null) throw new FileStorageException("手动分片上传-上传分片失败，请传入 fileInfo 参数");
        if (fileInfo.getPlatform() == null)
            throw new FileStorageException("手动分片上传-上传分片失败，请在 FileInfo 中传入 platform 参数");
        if (fileInfo.getBasePath() == null)
            throw new FileStorageException("手动分片上传-上传分片失败，请在 FileInfo 中传入 basePath 参数");
        if (fileInfo.getPath() == null) throw new FileStorageException("手动分片上传-上传分片失败，请在 FileInfo 中传入 path 参数");
        if (fileInfo.getFileName() == null)
            throw new FileStorageException("手动分片上传-上传分片失败，请在 FileInfo 中传入 filename 参数");
    ...
    public static void completeMultipartUpload(FileInfo fileInfo) {
        if (fileInfo == null) throw new FileStorageException("手动分片上传-完成失败，请传入 fileInfo 参数");
        if (fileInfo.getPlatform() == null)
            throw new FileStorageException("手动分片上传-完成失败，请在 FileInfo 中传入 platform 参数");
        if (fileInfo.getBasePath() == null)
            throw new FileStorageException("手动分片上传-完成失败，请在 FileInfo 中传入 basePath 参数");
        if (fileInfo.getPath() == null) throw new FileStorageException("手动分片上传-完成失败，请在 FileInfo 中传入 path 参数");
        if (fileInfo.getFileName() == null)
            throw new FileStorageException("手动分片上传-完成失败，请在 FileInfo 中传入 filename 参数");
    ...
    public static void abortMultipartUpload(FileInfo fileInfo) {
        if (fileInfo == null) throw new FileStorageException("手动分片上传-取消失败，请传入 fileInfo 参数");
        if (fileInfo.getPlatform() == null)
            throw new FileStorageException("手动分片上传-取消失败，请在 FileInfo 中传入 platform 参数");
        if (fileInfo.getBasePath() == null)
            throw new FileStorageException("手动分片上传-取消失败，请在 FileInfo 中传入 basePath 参数");
        if (fileInfo.getPath() == null) throw new FileStorageException("手动分片上传-取消失败，请在 FileInfo 中传入 path 参数");
        if (fileInfo.getFileName() == null)
            throw new FileStorageException("手动分片上传-取消失败，请在 FileInfo 中传入 filename 参数");
        if (fileInfo.getUrl() == null) throw new FileStorageException("手动分片上传-取消失败，请在 FileInfo 中传入 url 参数");
    ...
    public static void listParts(FileInfo fileInfo) {
        if (fileInfo == null) throw new FileStorageException("手动分片上传-列举已上传的分片失败，请传入 fileInfo 参数");
        if (fileInfo.getPlatform() == null)
            throw new FileStorageException("手动分片上传-列举已上传的分片失败，请在 FileInfo 中传入 platform 参数");
        if (fileInfo.getBasePath() == null)
            throw new FileStorageException("手动分片上传-列举已上传的分片失败，请在 FileInfo 中传入 basePath 参数");
        if (fileInfo.getPath() == null)
            throw new FileStorageException("手动分片上传-列举已上传的分片失败，请在 FileInfo 中传入 path 参数");
        if (fileInfo.getFileName() == null)
            throw new FileStorageException("手动分片上传-列举已上传的分片失败，请在 FileInfo 中传入 filename 参数");
    ...
    public static void listFiles(ListFilesPretreatment pre) {
        if (pre.getPlatform() == null) throw new FileStorageException("列举文件失败，请传入 platform 参数");
        if (pre.getPath() == null) throw new FileStorageException("列举文件失败，请传入 path 参数");
        if (pre.getFilenamePrefix() == null) throw new FileStorageException("列举文件失败，请传入 filenamePrefix 参数");
    }
    ...
    public static void getFile(GetFilePretreatment pre) {
        if (pre.getPlatform() == null) throw new FileStorageException("获取文件失败，请传入 platform 参数");
        if (pre.getPath() == null) throw new FileStorageException("获取文件失败，请传入 path 参数");
        if (pre.getFilename() == null) throw new FileStorageException("获取文件失败，请传入 filename 参数");
        // if (pre.getUrl() == null) throw new FileStorageException("获取文件失败，请传入 url 参数");
    }
    ...
    public static void generatePresignedUrl(GeneratePresignedUrlPretreatment pre) {
        if (pre.getPlatform() == null) throw new FileStorageException("生成预签名 URL 失败，请传入 platform 参数");
        if (pre.getExpiration() == null) throw new FileStorageException("生成预签名 URL 失败，请传入 expiration 参数");
        if (pre.getMethod() == null) throw new FileStorageException("生成预签名 URL 失败，请传入 method 参数");
    }
}
