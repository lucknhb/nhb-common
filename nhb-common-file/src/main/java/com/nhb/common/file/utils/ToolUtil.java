package com.nhb.common.file.utils;
...
public class ToolUtil {
    ...
    public static <K, V, NK, NV> Map<NK, NV> stream(
        ...
        if (mapper == null) {
            mapper = (nknvMap, kvEntry) -> nknvMap.put(cast(kvEntry.getKey()), cast(kvEntry.getValue()));
        }
        if (callback == null) {
    ...
    public static <K, V, NK, NV> Map<NK, NV> stream(
        ...
        if (keyMapper == null) {
            keyMapper = e -> cast(e.getKey());
        }
        if (valueMapper == null) {
            valueMapper = e -> cast(e.getValue());
        }
