...
import com.nhb.common.mybatis.handler.DataPermissionPlusHandler;
 ...
 * @author luck_nhb
 * @version 1.0
 * @date 2026/3/19 16:57
 * @description: 数据权限拦截器
 */
@Slf4j
public class DataPermissionInterceptor extends BaseMultiTableInnerInterceptor implements InnerInterceptor {
    private final DataPermissionPlusHandler dataPermissionPlusHandler = new DataPermissionPlusHandler();
    ...
    public void beforeQuery(Executor executor, MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) throws SQLException {
        ...
        // 检查是否缺少有效的数据权限注解
        if (dataPermissionPlusHandler.invalid()) {
            return;
        }
        // 解析 sql 分配对应方法
    ...
    protected void processUpdate(Update update, int index, String sql, Object obj) {
        Expression sqlSegment = dataPermissionPlusHandler.getSqlSegment(update.getWhere(), false);
        if (null != sqlSegment) {
    ...
    protected void processDelete(Delete delete, int index, String sql, Object obj) {
        Expression sqlSegment = dataPermissionPlusHandler.getSqlSegment(delete.getWhere(), false);
        if (null != sqlSegment) {
    ...
    protected void setWhere(PlainSelect plainSelect, String mappedStatementId) {
        Expression sqlSegment = dataPermissionPlusHandler.getSqlSegment(plainSelect.getWhere(), true);
        if (null != sqlSegment) {
    ...
    public Expression buildTableExpression(Table table, Expression where, String whereSegment) {
        // 只有新版数据权限处理器才会执行到这里
        final MultiDataPermissionHandler handler = (MultiDataPermissionHandler) dataPermissionPlusHandler;
