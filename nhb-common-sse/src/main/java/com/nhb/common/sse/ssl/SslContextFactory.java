package com.nhb.common.sse.ssl;
...
public class SslContextFactory {
     ...
     * 创建SSL上下文
     *
     * @param sseConfigProperties 配置文件
     * @param resourceLoader            加载器
     * @return SSL上下文
     * @throws Exception 异常信息
     */
    public static SslContext create(SseConfigProperties sseConfigProperties,
                                    ResourceLoader resourceLoader) throws Exception {
        SseConfigProperties.Ssl ssl = sseConfigProperties.getSsl();
        ...
        if (StringUtil.isNotBlank(ssl.getCertFilePath()) && StringUtil.isNotBlank(ssl.getKeyFilePath())) {
            return createFromFiles(sseConfigProperties, resourceLoader);
        }
        ...
        if (Boolean.TRUE.equals(ssl.getEnabled())) {
            ...
            return createFromGenerated(sseConfigProperties);
        }
     ...
     * 外部证书
     *
     * @param sseConfigProperties 配置项
     * @param resourceLoader            加载器
     * @return SSL上下文
     * @throws Exception 异常消息
     */
    private static SslContext createFromFiles(SseConfigProperties sseConfigProperties,
                                              ResourceLoader resourceLoader) throws Exception {
        SseConfigProperties.Ssl ssl = sseConfigProperties.getSsl();
     ...
     * @throws Exception 异常信息
     */
    private static SslContext createFromGenerated(SseConfigProperties sseConfigProperties) throws Exception {
        //重复使用已存在的证书信息
     ...
     * @param ssl     ssl配置项
     */
    private static void applySettings(SslContextBuilder builder, SseConfigProperties.Ssl ssl) {
        // SSL 提供者
